...
for (var i = 0; i< hertzArray.length; i++) {
  var url = "https://www.hertz.com/rentacar/vehicleguide/index.jsp?targetPage=vehicleGuideHomeView.jsp&countryCode=US&category=" + hertzArray[i];

  axios.get(url).then(function (res) {
    var $ = cheerio.load(res.data);
    
    $(".vgVehicle").each(function (i) {
      var model = $(this).find(".vgSippDescription").text().trim();
      // var imgSrc = $(this).find("img").attr("src").trim();
      // var transmission = $(this).find("li.sx-equipment-transmission").text();
      // var baggageSpace = $(this).find("li.sx-equipment-case_big").text().trim();
      // var people = $(this).find("li.sx-equipment-seats").text().trim();
      model = model !== "Reserve this Exact Model" ? model : "";
      if(model!==undefined){
        var car = {
          model: model 
